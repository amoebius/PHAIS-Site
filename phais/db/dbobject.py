...
class DBObject(object):
	...
	def _dbo_load(self, **kwargs):
		...
		with db as c:
			...
			if kwargs:
				c.execute('SELECT * FROM %s WHERE ' + ' AND '.join('%s=' + self._dbo_format(value) for value in kwargs.values()), self._dbo_table, *[value for item in kwargs.items() for value in item])
			else:
	...
	def _dbo_save(self):
		...
		with db as c:
			...
			c.execute('UPDATE %s SET ' + ', '.join('%s=' + self._dbo_format(value) for value in data.values()), self._dbo_table, *[value for item in data.items() for value in item])


	def _dbo_format(self, item):
		if type(item) is int:
			return '%d'
		else:
			return "'%s'"